...
{ // смотрит по номеру UART c каким будет идти работа
//	uint8_t result;
...
{ //читаем у нас пришла единица или ноль в ответ
...
{ //
	...
	while (recvFlag & (1 << i) && pause--)
		; // ждем пока кто-то не ответит но не больше паузы
	return rc_buffer[i]; //в зависимости от используемого номера UART
}

uint8_t* byteToBits(uint8_t ow_byte, uint8_t *bits)
{ //разлагаем 1 байт на 8 байт ,кодируем так скасказать в посылку для 1wire
	uint8_t i;
	for (i = 0; i < 8; i++)
	{
		if (ow_byte & 0x01)
...
{ //принимает "кодированый" массив байт полученный по UART и делает из него байт))
...
{ //
	...
	do
	{
		...
		if (cB != cmp_cB)
		{ //нормальная ситуация пришло либо 10 либо 01
			searchDirection = cB; //выбираем в каком направлении будем двигатся дальше
		}
		else
		{ //колизия пришло 00 т.е текущий бит у ROM-ов разный
			...
			else
			{
				...
				{ //если мы зашили дальше
					searchDirection = 0; //выбираем в каком направлении будем двигатся дальше
				}
				else
				{
					searchDirection = (uint8_t) ((ow->lastROM[byteNum]
							>> ui32BitNumber % 8) & 0x01);
				}
...
{ //отправляет команду пропуска ROM после этого следующая команда будет
	owResetCmd();                 //для всех устройств на шине
	owSendByte(ONEWIRE_SKIP_ROM);
}

void owMatchRomCmd(RomCode *rom)
{              //позволяет мастеру обращаться к конкретному  ведомому устройству
	int i = 0;
	owResetCmd();
...
void owWriteDS18B20Scratchpad(OneWire *ow, RomCode *rom, uint8_t th, uint8_t tl,
		uint8_t conf)
{
...
int get_ROMid(void)
{
	...
	{    // is anybody on the bus?
		...
		i = 0;
		for (; i < devices; i++)
		{        //выводим в кон�?оль в�?е найденные ROM
			RomCode *r = &ow.ids[i];
